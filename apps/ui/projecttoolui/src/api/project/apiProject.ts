import { IProject, IProjectBase, IRole, utilFixIProject, utilValidateIProject } from "@frosttroll/projecttoolmodels";
...
export async function apiPostCreateProject(projectData: Partial<IProject>): Promise<void> {
    const rest = RestService.getInstance();
    await rest.POST<Partial<IProject>>("/project", parsePartialProjectObjectBeforeSaving(projectData));
...
export async function apiPostUpdateProject(projectData: IProject): Promise<IProject> {
    const rest = RestService.getInstance();
    const res = await rest.POSTwithResponse<IProject>(
        `/project/${projectData.guid}`,
        parseProjectObjectBeforeSaving(projectData)
    );
    if (res) {
        return Promise.resolve(fixProjectResponse(res as IProject));
    }
...
function parseProjectObjectBeforeSaving(project: IProject): IProject {
    const clearedProject: IProject = { ...project };

    // remove roletemplates stored to roles
    clearedProject.roles = clearedProject.roles.map((role) => {
        const clearedRole = { ...role };
        if ("template" in clearedRole) {
            delete (clearedRole as IRole).template;
        }
        return clearedRole;
    });
...
function parsePartialProjectObjectBeforeSaving(project: Partial<IProject>): Partial<IProject> {
    const clearedProject: Partial<IProject> = { ...project };

    // remove roletemplates stored to roles
    if (clearedProject.roles) {
        clearedProject.roles = clearedProject.roles.map((role) => {
            const clearedRole = { ...role };
            if ("template" in clearedRole) {
                delete (clearedRole as IRole).template;
