...
import { Autocomplete, Box, Button, Card, Container, Divider, Flex, Stack, Text, Title } from "@mantine/core";
...
function TextsComponent() {
    ...
    return (
        <DataShell>
            <Container size="xl">
                <ProjectPageMainTitle>Texts</ProjectPageMainTitle>
        ...
        <Card padding="xs" onClick={() => props.onClick(txt)} style={{ cursor: "pointer" }}>
            <Flex align="center" justify="flex-start" mb="sm" style={{ borderBottom: "solid 1px #8882" }}>
                ...
                <Text size="xs" ml="xs" style={{ width: "250px" }}>
                    {txt.keywords && txt.keywords.length > 0
                        ? txt.keywords.slice(0, 5).join(", ").toLocaleLowerCase()
                        : "No keywords"}
                </Text>

                <Text size="xs" ml="xs" style={{ width: "50px" }}>
                    {txt.langlinks && Object.keys(txt.langlinks).length > 0
                        ? Object.keys(txt.langlinks).join(", ")
                        : "-"}
                </Text>
            </Flex>
        </Card>
