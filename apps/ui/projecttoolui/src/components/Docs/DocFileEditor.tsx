...
import { EDOCITEMTYPE, IDocFile, IDocFileContent, IDocFileHeader, IDocFileHtml, IDocFileParagraph, IText } from "@frosttroll/projecttoolmodels";
...
const DocFileEditor = (props: IDocFileEditorProps) => {
	...
	return (
		<Fieldset legend="Doc Item Editor" radius="md">
			{item.type === EDOCITEMTYPE.HEADER && <DocFileEditorHeader {...props} contentItem={item} onSave={(item) => setItem(item as IDocFileContent)} />}
			{item.type === EDOCITEMTYPE.UNKNOWN && <DocFileEditorUnknown {...props} contentItem={item} onSave={(item) => setItem(item as IDocFileContent)} />}
			{item.type === EDOCITEMTYPE.PARAGRAPH && (
				<DocFileEditorParagraph {...props} contentItem={item} onSave={(item) => setItem(item as IDocFileContent)} />
			)}
			{item.type === EDOCITEMTYPE.HTML && <DocFileEditorHtml {...props} contentItem={item} onSave={(item) => setItem(item as IDocFileContent)} />}
			<Flex align="center" justify="space-between" gap="md" mt="md">
				<ConfirmButton variant="filled" color="red" onClick={() => props.onDelete()} leftSection={<IconTrash />} question={`Delete this doc item?`}>
					Delete
				</ConfirmButton>
				<Group>
...
const DocFileEditorHeader = (props: IDocFileEditorProps) => {
	...
	return (
		<Stack gap="sm">
			<TextInput label="Header Text" description="Edit the header text" value={header.text} onChange={(e) => handleEdit(e.currentTarget.value)} />
			<Select
				label="Header Level"
