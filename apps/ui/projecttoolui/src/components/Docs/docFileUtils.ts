 ...
 * @returns
 */
export function docGenerateProjectPlanContent(
    project: IProject,
    lang: EDOCLANG,
    preContent?: IDocFileContent[]
): IDocFileContent[] {
    const content: IDocFileContent[] = preContent
        ? [...preContent]
        : lang === EDOCLANG.EN
          ? [...PROJECTPLAN_TEMPLATE_LARGE_EN]
          : [...PROJECTPLAN_TEMPLATE_LARGE_FI];
    ...
    const insertIndex = content.findIndex(
        (item) => item.type === EDOCITEMTYPE.HEADER && (item as IDocFileHeader).targetingKey === "phases"
    );
    if (insertIndex !== -1) {
