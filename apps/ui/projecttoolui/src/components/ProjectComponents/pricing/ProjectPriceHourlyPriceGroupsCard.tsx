...
const ProjectPriceHourlyPriceGroupsCard = () => {
    ...
    const [sliderScope, setSliderScope] = useState<{ min: number; max: number }>({ min: 30, max: 220 });
                                ...
                                <Box style={{ flex: "0 0 auto", width: "5%" }}>
                                    {!prg.permanent && (
                                        <Button
                                            variant="filled"
                                            color="red"
                                            onClick={() => handleRemovePriceGroup(prg.guid)}
                                        >
                                            <IconTrash />
                                        </Button>
                                    )}
...
const PriceSlider = (props: PriceSliderProps) => {
    ...
    return (
        <Slider {...sliderProps} value={value} onChange={(val) => setValue(val)} onChangeEnd={(val) => onDone(val)} />
    );
};
