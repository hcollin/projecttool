...
const EndTimeEditor = (props: { phase: IPhase; project: IProject; onSave: (start: IPhaseStart) => void; onCancel: () => void }) => {
	...
	return (
		<>
			...
			<Flex align="center" justify="space-between" gap="md" style={{ height: "5rem" }} wrap="wrap">
				<Box>
					<Switch label="At project end" checked={atPrjEnd} onChange={(event) => handleToggleAtProjectEnd(event.currentTarget.checked)} />
				</Box>
				...
				<Box>
					<NumberInput
						...
						value={lengthInWDays}
						onChange={(value) => handleWorkingDaysChange(typeof value === "string" ? parseInt(value) : value)}
					/>
				</Box>
